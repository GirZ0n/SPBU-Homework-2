...
internal class ParsingTreeTest {
    ...
    fun calculate_NumberThenNumber_MustWork() {
        ...
        parsingTree.parseFile(ByteArrayInputStream("(+ 3 4)".toByteArray()))
    ...
    fun calculate_NumberThenBrackets_MustWork() {
        val parsingTree = ParsingTree()
        parsingTree.parseFile(ByteArrayInputStream("(- 3 (* 4 10))".toByteArray()))
    ...
    fun calculate_BracketsThenNumber_MustWork() {
        val parsingTree = ParsingTree()
        parsingTree.parseFile(ByteArrayInputStream("(/ (* 3 13) 3)".toByteArray()))
    ...
    fun calculate_BracketsThenBrackets_MustWork() {
        val parsingTree = ParsingTree()
        parsingTree.parseFile(ByteArrayInputStream("(* (+ 3 13) (/ 144 12))".toByteArray()))
    ...
    fun calculate_ComplexTreeWithAllOperations_MustWork() {
        val parsingTree = ParsingTree()
        parsingTree.parseFile(ByteArrayInputStream("(* (+ (- 2 5) (/ 10 2)) (/ (+ 43 5) (* 2 6)))".toByteArray()))
        assertEquals(8, parsingTree.calculate())
    }

    @Test
    fun calculate_ComplexTreeWithLotsOfBrackets() {
        val parsingTree = ParsingTree()
        parsingTree.parseFile(ByteArrayInputStream("(* (+ 1 (+ 1 (+ (+ (+ 1 (+ (+ 1 1) 1)) 1) 1))) 10)".toByteArray()))
